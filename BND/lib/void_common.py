...
def open_star_unlock():
    ...
    if s.lang == "de":
        panel(
            "STAR ZUM FREISCHALTEN",
            "Star das GitHub Repo um BND-TOOLS zu unterstützen und Premium freizuschalten.",
        )
    else:
        panel(
            "STAR FOR UNLOCK",
            "Star the GitHub repo to support BND-TOOLS and unlock premium.",
        )
    try:
        webbrowser.open(C.DISCORD)
        time.sleep(0.4)
...
def append_star_unlock_items(items):
    if not items:
        return items
    s = get_settings()
    label = "Star zum Freischalten [STAR]" if s.lang == "de" else "Star for unlock [STAR]"
    base = len(items)
    extra = []
    for i in range(4):
        code = f"{base + i + 1:02d}"
...
def pause(msg=None):
    ...
    if msg is None:
        msg = (
            f"\033[38;2;120;0;0m  ► Enter drücken zum Fortfahren…\033[0m"
            if s.lang == "de"
...
def safe_action(fn, tool_name: str = "Module"):
    try:
        ...
        error_box("File not found", str(e), "Check BND-TOOLS installation.")
        time.sleep(2)
    except subprocess.CalledProcessError as e:
        error_box("Execution failed", tool_name, f"exit code: {e.returncode}")
        time.sleep(2)
