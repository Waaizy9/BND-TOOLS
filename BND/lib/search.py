...
def parse_filters(raw: str):
    ...
    return tier, cat, " ".join(search_terms) if search_terms else raw.strip()
...
def run_search_ui(router, live):
    ...
    console.print(Panel(
        Align.center(Text.from_markup(
            f"[{C.C_GOLD} bold]{'SEARCH' if s.lang == 'de' else 'SEARCH'}[/]\n[{C.C_DIM}]{hint}[/]"
        )),
    ...
    if not matches:
        error_box("No results" if s.lang == "de" else "No results", q, "free: token · cat:social")
        pause()
        cls()
        live.start()
        return
    ...
    pick = input(f"\n{ansi_hex(C.C_MID)}  # (Enter = back) >> \033[0m").strip()
